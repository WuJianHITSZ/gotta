...
FindSpatialNeighbors <- function(object,images){
...
RunGOTT <- function(object, sample_id = NULL, reduction_name = "GOTT", shape = "disk", aspect_ratio = 1, assay = NULL, features = NULL, reduction = "coordinate", n.components = NULL, select_points = FALSE){
  ...
  if(select_points){
    plot_Spatial <- MeshFeaturePlot(object, features = "cluster")
  ...
  system("/Applications/MATLAB_R2022a.app/bin/matlab -nodisplay -r \"run('demo_gott_r.m'); exit\"")

...
FindSpatialNeighbors <- function(object,images,staffli){
  ...
  face_area <- vector(mode = "numeric", length = nrow(face))
  face_area <- 0 # initialization
  ...
  setClass("Mesh", slots = list(face = "matrix", boundary = "numeric", corners = "numeric", face_area = "numeric"))
  object@tools[["mesh_Spatial"]] <- new("Mesh", face = face, boundary = boundary, corners = boundary[1], face_area = face_area)    
...
RunGOTT <- function(object, sample_id = NULL, reduction_name = "GOTT", shape = "disk", aspect_ratio = 1, assay = NULL, features = NULL, reduction = "coordinate", n.components = NULL, select_points = FALSE){
  ...
  object@tools[["mesh_Spatial"]]@face_area <- ComputeFaceArea(face, vertex)
  
  object$vertex_area <- ComputeVertexArea(face, vertex)
